...
centos_kernel_buildid=.yn20201125
...
case "$centos_major" in
	7)
		...
		centos_kernel_revision_default=1160.6.1.el7
...
if [ -n "$centos_use_git" ]; then
	...
	PKG_SOURCE_MD5SUM=64289db233697a9f797b0053bc364449
fi
...
do_patch() {
	...
	for config in *.config; do
		...
		kconfig_set_option CONFIG_SCSI_HPSA m "$config"
		kconfig_set_option CONFIG_SCSI_MPT2SAS m "$config"
		kconfig_set_option CONFIG_SCSI_MPT3SAS m "$config"
		kconfig_set_option CONFIG_MEGARAID_NEWGEN y "$config"
		kconfig_set_option CONFIG_MEGARAID_MM m "$config"
		kconfig_set_option CONFIG_MEGARAID_MAILBOX m "$config"
		kconfig_set_option CONFIG_MEGARAID_LEGACY m "$config"
		kconfig_set_option CONFIG_MEGARAID_SAS m "$config"

		kconfig_set_option CONFIG_FUSION_FC n "$config"
		kconfig_set_option CONFIG_FUSION_SPI m "$config"
		kconfig_set_option CONFIG_FUSION_SAS m "$config"
		kconfig_set_option CONFIG_FUSION_CTL m "$config"
		kconfig_set_option CONFIG_FUSION_LOGGING y "$config"

		kconfig_set_option CONFIG_HOTPLUG_PCI y "$config"
		kconfig_set_option CONFIG_HOTPLUG_PCI_ACPI y "$config"
		kconfig_set_option CONFIG_HOTPLUG_PCI_ACPI_IBM m "$config"
		kconfig_set_option CONFIG_HOTPLUG_PCI_CPCI n "$config"
		kconfig_set_option CONFIG_HOTPLUG_PCI_SHPC y "$config"

		kconfig_set_option CONFIG_SND_SST_ATOM_HIFI2_PLATFORM m "$config"
		kconfig_set_option CONFIG_SND_SOC_INTEL_SST_MATCH m "$config"
		kconfig_set_option CONFIG_SND_SOC_INTEL_BXT_DA7219_MAX98357A_MACH m "$config"
		kconfig_set_option CONFIG_SND_SOC_INTEL_BXT_RT298_MACH m "$config"
		kconfig_set_option CONFIG_SND_SOC_INTEL_KBL_RT5663_MAX98927_MACH m "$config"
		kconfig_set_option CONFIG_SND_SOC_INTEL_KBL_RT5663_RT5514_MAX98927_MACH m "$config"
		kconfig_set_option CONFIG_SND_SOC_INTEL_SKYLAKE m "$config"
		kconfig_set_option CONFIG_SND_X86 y "$config"

		kconfig_set_option CONFIG_TYPEC m "$config"
		kconfig_set_option CONFIG_TYPEC_UCSI m "$config"
